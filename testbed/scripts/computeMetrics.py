...
# computeMetrics.py 
#
# Usage: python computeMetrics.py policy
#        where   policy = SELinux policy
#        The program outputs m1, m2 and m3 metric results
#
# Example: 
#          python computeMetrics.py RBAC001.te
#

import sys
import re

rex = re.compile(r'(?P\d+\.\d)')

#========================================================================
def main():
   if len(sys.argv) < 1:
       sys.exit('Usage: %s policy' % sys.argv[0])
   else: 
